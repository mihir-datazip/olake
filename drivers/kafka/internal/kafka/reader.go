...
func NewReaderManager(config ReaderConfig) *ReaderManager {
	return &ReaderManager{
		...
		partitionMeta: make(map[string]PartitionMetaData),
	}
}
...
func (r *ReaderManager) CreateReaders(ctx context.Context, streams []types.StreamInterface) error {
	r.partitionMeta = make(map[string]PartitionMetaData)
...
func (r *ReaderManager) GetPartitionMeta(partitionMetadataKey string) (PartitionMetaData, bool) {
...
func (r *ReaderManager) PartitionsForStream(ctx context.Context, stream types.StreamInterface) (map[string]PartitionMetaData, error) {
	topic := stream.Name()
	...
	partitionsMetadata := make(map[string]PartitionMetaData)
	for _, partitionDetail := range topicDetail.Partitions {
		...
		partitionsMetadata[PartitionMetadataKey(topic, partitionDetail.Partition)] = PartitionMetaData{
			Stream:          stream,
