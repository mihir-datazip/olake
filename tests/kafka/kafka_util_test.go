...
// sourceConfig mirrors the connection fields of the driver's source.json.
...
func ExecuteQueryJSON(ctx context.Context, t *testing.T, streams []string, operation string, fileConfig bool) {
	...
	if fileConfig {
		...
		testutils.UnmarshalFile("./testdata/source.json", &config)
...
func startRebalanceTrigger(ctx context.Context, t *testing.T, topic string) {
	...
	go func() {
		...
		client, err = kgo.NewClient(
			...
			kgo.Balancers(newRebalanceBalancer(topic, rebalanceBulkPartition)),
...
func waitForSyncProgress(ctx context.Context, t *testing.T) {
	...
	statsPath := testutils.GetTestConfig("kafka", "json").HostStatsPath
	require.Eventually(t, func() bool {
		...
		if err := testutils.UnmarshalFile(statsPath, &stats); err != nil {
			return false
		}
