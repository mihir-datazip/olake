...
// built by `make docker.build.<driver>`; OLAKE_DRIVER_IMAGE overrides it.
...
// `make docker.build.<driver>` when missing. CI pre-builds the image; this fallback keeps
...
func ensureDriverImage(t *testing.T, cfg *TestConfig) string {
	...
	ensureImageOnce.Do(func() {
		...
		t.Logf("driver image %s not found locally, building it with `make docker.build.%s`", ref, cfg.Driver)
		defer trackPhaseTiming(t, "driver-image", ref)()
		cmd := exec.Command("make", fmt.Sprintf("docker.build.%s", cfg.Driver))
